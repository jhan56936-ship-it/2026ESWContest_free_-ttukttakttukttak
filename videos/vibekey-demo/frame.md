...
description: >
  ...
  accent, the three-step gray text ladder, Gothic A1 (display/numerals/chrome) + Noto Sans KR (body),
...
colors:
  ...
  text-muted: "#41505E"
  text-light: "#8B93A1"
  accent-light: "rgba(11, 87, 208, 0.08)"
  accent-medium: "rgba(11, 87, 208, 0.15)"
  border: "rgba(11, 87, 208, 0.2)"
  card-bg: "rgba(11, 87, 208, 0.04)"
  slot-1: "#0B57D0"   # 1번 버튼 — 코발트 (primary와 동일)
  slot-2: "#12703A"   # 2번 버튼 — 초록
  slot-3: "#B26A00"   # 3번 버튼 — 호박
  positive: "#12703A"
  negative: "#B3261E"
...
typography:
  # — reading ramp (Noto Sans KR body + Gothic A1 chrome) —
  body:    { fontFamily: "Noto Sans KR", cqw: 0.85, weight: 400, lineHeight: 1.6, color: "text-muted" }
  h4-eyebrow:{ fontFamily: "Gothic A1", cqw: 0.8, weight: 600, tracking: "0.08em", upper: true, color: "primary" }
  tag:     { fontFamily: "Gothic A1", px: 12, weight: 500, color: "primary" }
  counter: { fontFamily: "Gothic A1", px: 13, weight: 500, tracking: "0.05em", color: "text-muted" }
  # — display / numerical ramp (Gothic A1, near-black headings / cobalt numerals) —
  h3:      { fontFamily: "Gothic A1", cqw: 1.25, weight: 500, lineHeight: 1.3, tracking: "-0.02em", color: "text" }
  stat-num:{ fontFamily: "Gothic A1", cqw: 1.9, weight: 700, lineHeight: 1.0, color: "primary" }
  blockquote:{ fontFamily: "Gothic A1", cqw: 2.4, weight: 500, lineHeight: 1.35, color: "text" }
  h2:      { fontFamily: "Gothic A1", cqw: 2.6, weight: 600, lineHeight: 1.1, tracking: "-0.02em", color: "text" }
  metric-value:{ fontFamily: "Gothic A1", cqw: 3.0, weight: 700, lineHeight: 1.0, color: "primary" }
  h1:      { fontFamily: "Gothic A1", cqw: 4.2, weight: 700, lineHeight: 1.08, tracking: "-0.02em", color: "text" }
  quote-mark:{ fontFamily: "Gothic A1", cqw: 8.0, weight: 700, lineHeight: 0.5, color: "primary", opacity: 0.15 }
...
- **Fonts** — already set to **Gothic A1** (display) / **Noto Sans KR** (body); ignore any preset font name lingering in prose.
...
The voice is two faces in fixed roles: **Gothic A1** (display, every numeral, all chrome —
...
- **Gothic A1** (display/numerals/chrome) + **Noto Sans KR** (body) — near-black headlines, cobalt numerals.
...
Two ramps. The **reading ramp** (Noto Sans KR body 0.85cqw muted; Gothic A1 eyebrow uppercase 0.08em
cobalt) carries copy + chrome; the **display/numerical ramp** (Gothic A1 `h3` 1.25cqw → `h1`
...
- **Headlines near-black, −0.02em**; **eyebrows cobalt, uppercase, 0.08em**; **numerals cobalt 600–700**; **body Gothic A1 400 muted, line 1.6**. No italic, no uppercase body, no cobalt headline.
...
**Composes** accent-line, meta, h1, body sub. **Focal** a 2-line Gothic A1 `h1` near-black, left,
...
**Focal** a row of tinted cards — cobalt `metric-value` + Gothic A1 label + muted desc + optional
...
blockquote, cite. **Focal** a Gothic A1 `blockquote` near-black under a 15%-opacity cobalt
...
Gothic A1 body column beside a cobalt-8% highlight block (4px cobalt left rule) carrying an inline pull
...
a Gothic A1 `h1` near-black, centered, with the one solid cobalt `cta-button` pill below. **Accent**
...
- Use **tinted cards** (4% fill, 20% border, 10–14px radius, no shadow); body Gothic A1 400 muted, line 1.6.
...
- **Type** — Gothic A1 headings −0.02em near-black, cobalt eyebrows 0.08em + numerals; Gothic A1 body muted line 1.6; ≥1.4cqw floor.
...
- **Gothic A1 + Gothic A1 via Google Fonts.** CJK pairing (Noto Sans SC 700 display / Noto Serif SC 400 body) carries over; the eyebrow's uppercase+tracking signal weakens in CJK — pair it with the accent-line.
...
## 폰트 파일 (렌더 머신에는 폰트가 없다 — 반드시 이 파일을 @font-face 로 건다)

| 역할 | family | 파일 | weight |
|---|---|---|---|
| display / 숫자 / 크롬 | `Gothic A1` | `assets/fonts/GothicA1-Regular.ttf` | 400 |
|  |  | `assets/fonts/GothicA1-Medium.ttf` | 500 |
|  |  | `assets/fonts/GothicA1-SemiBold.ttf` | 600 |
|  |  | `assets/fonts/GothicA1-Bold.ttf` | 700 |
| 본문 | `Noto Sans KR` | `assets/fonts/NotoSansKR.ttf` (가변 100–900) | 100–900 |
| 데이터 / 헥사 / 코드 | `JetBrains Mono` | `assets/fonts/JetBrainsMono.ttf` (가변 100–800) | 100–800 |

경로는 **프로젝트 루트 기준**으로만 쓴다 (`assets/fonts/...`). `../` 금지, 네트워크 `@import` 금지.
`JetBrains Mono` 는 라틴 전용이므로 **헥사·식별자·영문 라벨에만** 쓴다. 한글은 절대 넣지 않는다.
