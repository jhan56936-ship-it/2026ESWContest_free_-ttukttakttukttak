...
description: >
  ...
  borders, hard offset shadows (no blur), Gothic A1 800 uppercase at 0.92 line-height,
  JetBrains Mono taxonomy, Gothic A1 body, the four-accent palette. Composition,
...
colors:
  ...
  ink: "#101418"
  ink-2: "#2A2A2A"
  green: "#12703A"        # 2번 단추 — 앱과 같은 초록
  green-dark: "#0C4F28"
  pink: "#8A2BE2"         # AI 강조 — 앱의 보라
  pink-dark: "#6F1FB8"
  orange: "#B26A00"       # 3번 단추 — 앱과 같은 호박
  yellow: "#0B57D0"       # 1번 단추 — 앱과 같은 코발트

typography:
  # — reading ramp (px @ 1920, with cqw) —
  body-lg:    { fontFamily: "Gothic A1", px: 28, cqw: 1.46, weight: 400, lineHeight: 1.4 }
  body-md:    { fontFamily: "Gothic A1", px: 24, cqw: 1.25, weight: 400, lineHeight: 1.3 }
  mono-label: { fontFamily: "JetBrains Mono", px: 24, cqw: 1.25, weight: 400, tracking: "0.06em" }
  mono-kicker:{ fontFamily: "JetBrains Mono", px: 24, cqw: 1.25, weight: 400, tracking: "0.14em" }
  table-head: { fontFamily: "Gothic A1", px: 28, cqw: 1.46, weight: 800 }
  # — display / hero ramp (frame-native, cqw-first) —
  step-title: { fontFamily: "Gothic A1", px: 34,  cqw: 1.77, weight: 800, lineHeight: 1.0 }
  badge-label:{ fontFamily: "Gothic A1", px: 28,  cqw: 1.46, weight: 800 }
  marker:     { fontFamily: "Gothic A1", cqw: 2.4, weight: 800, lineHeight: 1.0 }
  stamp-num:  { fontFamily: "Gothic A1", cqw: 2.2, weight: 800, lineHeight: 0.9 }
  stat-num:   { fontFamily: "Gothic A1", cqw: 6.4, weight: 800, lineHeight: 0.88 }
  step-num:   { fontFamily: "Gothic A1", cqw: 7.0, weight: 800, lineHeight: 0.85 }
  display-head: { fontFamily: "Gothic A1", cqw: 4.2, weight: 800, lineHeight: 0.92, tracking: "-0.01em" }
  display-lg: { fontFamily: "Gothic A1", cqw: 8.0,  weight: 800, lineHeight: 0.9,  tracking: "-0.01em" }
  display-xl: { fontFamily: "Gothic A1", cqw: 11.0, weight: 800, lineHeight: 0.9,  tracking: "-0.01em" }
  display-hero:{ fontFamily: "Gothic A1", cqw: 15.5,weight: 800, lineHeight: 0.84, tracking: "-0.02em" }
...
The display voice is **Gothic A1 800 in strict uppercase at 0.92 line-height** — letters overlap
their own cap height; that tightness is the brand. **JetBrains Mono** carries every label, kicker,
counter, and axis as a "technical artifact" register. **Gothic A1** carries the few body lines.
...
- **Gothic A1 800 uppercase**, 0.92 line-height, always; sentence-case Gothic A1 800 does not exist.
...
- **Uppercase + 0.92 line-height on all Gothic A1 800**, always. Mono carries 0.06–0.14em tracking. Never letter-space Gothic A1 800 beyond the encoded −0.01em; never set body centered.
...
**Fixed** Gothic A1 800 uppercase 0.84 lh, one accent on the wordmark, square corners. **Free** which
...
- Size headlines **fit-to-measure**; render Gothic A1 800 uppercase at 0.92 lh.

### Don't

- Don't round corners (except the pill chip); don't gradient, blur, or glow.
- Don't set Gothic A1 800 in sentence case or letter-space it beyond −0.01em.
...
- **Type** — Gothic A1 800 uppercase 0.92 lh, fit-to-measure, ≥1.4cqw on load-bearing lines.
...
- **Gothic A1 800 requires Google Fonts**; fallback is `sans-serif`. CJK pairing (Noto Serif SC 900 / NSC 400) carries over from the source's CJK section.
...
## 한글 적용 메모 (이 프로젝트에서 손으로 고친 것)

프리셋의 자동 remix 뒤에 세 가지를 바로잡았다. 워커는 아래를 지킨다.

1. **디스플레이 서체는 Gothic A1 800.** 프리셋 원본의 Archivo Black은 라틴 전용이라
   한글이 한 글자도 안 나온다. 본문은 Gothic A1 400, 긴 문장은 Noto Sans KR.
2. **대문자 강제(`upper`)를 전부 걷어냈다.** 한글에는 효과가 없고 라틴 낱말만 들쭉날쭉해진다.
3. **`green` / `orange` / `yellow` / `pink` 는 앱과 같은 값으로 되돌렸다.**
   이 영상은 "기기의 단추 색 = 앱 화면의 색"이 전체를 꿰는 장치라, 색이 어긋나면 개념이 무너진다.

   | 토큰 | 값 | 뜻 |
   |---|---|---|
   | `yellow` | `#0B57D0` | 1번 단추 (코발트) |
   | `green` | `#12703A` | 2번 단추 (초록) |
   | `orange` | `#B26A00` | 3번 단추 (호박) |
   | `pink` | `#8A2BE2` | AI 강조 (보라) |

   토큰 **이름**은 프리셋 구조를 지키려고 그대로 두었다. 값만 브랜드다.
