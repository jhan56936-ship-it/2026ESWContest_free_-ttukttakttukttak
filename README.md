...
|---|---|
| 화면을 안 보고 쓰는데 **눌린 걸 어떻게 압니까?** | 버튼마다 다른 진동 (작품명 그대로 *Vibe*key) |
| **됐는지 안 됐는지**는요? | 폰이 결과를 돌려주고 기기가 성공/실패 진동을 다르게 냅니다 |
| 잡음 한 번에 택시가 불리면요? | CRC16 + SEQ + ACK + 재전송. 깨진 신호는 실행되지 않습니다 |
| **폰이 죽으면** 그냥 플라스틱 아닌가요? | 세 번 다 못 받으면 기기가 긴 진동으로 알립니다 |
...
| 폰 → 기기 | `FEEDBACK` | 실행 성공/실패를 서로 다른 진동으로 |
...
- **양방향** — 실행 결과가 기기로 돌아가 손끝에 대답이 됩니다.
